...
def single_auction_view(request, id):
    ...
    bid = Bids.objects.filter(auction_listing=single_auction)
    if len(bid) > 0:
        bid = Bids.objects.filter(auction_listing=single_auction).order_by('-offered_price')[0].offered_price
    else:
        bid = single_auction.starting_bid
    if bid_form.is_valid():
        inst = bid_form.save(commit=False)
        inst.created_by = request.user
        inst.auction_listing = single_auction
        print(bid_form.cleaned_data)
    context = {
        'bid' : int(bid),
